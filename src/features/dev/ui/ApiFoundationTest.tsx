...
import { setCredentials, clearCredentials } from '@/features/auth/store';
import { axios } from '@/lib/axios';
import { cn } from '@/lib/utils';
import { Button } from '@/components/ui/button';

/** Hardcoded test endpoints (dev tool only, not production code) */
const TEST_API_ENDPOINT = '/books?limit=1';
const INVALID_TRIGGER_ENDPOINT = '/me';
...
export function ApiFoundationTest({
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['posts-test'],
    queryFn: async () => {
      const response = await axios.get(TEST_API_ENDPOINT);
  ...
  const trigger401 = async () => {
    try {
      console.log('Sending request to', INVALID_TRIGGER_ENDPOINT);
      Cookies.set('token', 'EXPIRED_OR_INVALID_TOKEN');
      await axios.get(INVALID_TRIGGER_ENDPOINT);
    } catch (error: unknown) {
      const status =
        error instanceof Error && 'response' in error
          ? (error as { response?: { status?: number } }).response?.status
          : undefined;
      console.log('Error caught in test component:', status, error);
      setCookieValue(Cookies.get('token') || 'None');
    }
  };
